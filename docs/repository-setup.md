...
`eas.json` sets `cli.requireCommit=true` to bind native build evidence to committed source. It also sets `cli.appVersionSource=remote`; `app.json` keeps the reviewed native source version, currently Android `1.2.8` (`versionCode` 26), while future production store builds use `autoIncrement`.
