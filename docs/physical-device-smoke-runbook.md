...
| Encrypted offline retry/quarantine | Network loss queues driver events/proof media in SQLCipher; foreground+online retry preserves route order, applies backoff, and restart hydrates queue state. | Ordered evidence disappears, retries in background/offline, or storage opens without SQLCipher. |
