...
`cli.requireCommit` is enabled in `eas.json` so native evidence builds are tied to committed source. `cli.appVersionSource` is `remote`; the current local direct-distribution Android build is `1.1.3` (`versionCode` `10`), while future production store builds use `autoIncrement` to avoid duplicate build numbers.
