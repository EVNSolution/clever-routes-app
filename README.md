...
- live `EXPO_PUBLIC_DELIVERY_SERVER_BASE_URL` switch for route lookup, native secure storage for the short-lived driver token, and live downstream consent/assigned-route clients; background location services, GPS event streaming, stop actions, and proof-of-delivery left for follow-up slices
